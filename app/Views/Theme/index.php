            ...
            <div class="row row-cols-auto ">
                <?php foreach ($categorys as $category) : ?>
                    <div class="col-sm-1 border border-1 border-light p-3 text-center category">
                        <a href="<?php echo base_url('index_list/' . $category['category']); ?>" class="text-light fw-light" style="text-decoration: none;"><?php echo $category["category"]; ?></a>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
    <?php } else { ?>
        <h4 class="ps-3 text-warning bg-dark mb-0 py-2 rounded-top border border-1 border-light">ไม่พบข้อมูล</h4>
        <?php } ?>
</div>
